...
import {FlatList, StyleSheet, View,Text,ScrollView,Alert,TouchableOpacity,Image,Dimensions,TextInput,SafeAreaView,StatusBar,ImageBackground} from 'react-native';
...
export default class payFromCard extends Component {
  ...
  getCardList() {
    ...
    selectCard(card_id="",customer_id=""){
      
      Alert.alert(
        'Confirm',
        'Are you sure you want to pay from this card ?',
        [
          {
            text: 'Cancel',
          ...
          {text: 'OK', onPress: () => this.chargeAmount(card_id,customer_id)},
        ],
        {cancelable: false},
      );
    }
    chargeAmount(card_id="",customer_id=""){
      this.setState({ spinner: true })
      const { navigate } = this.props.navigation;
      const url = `${BASE_URL}/customer/create_charge`;
    const params = JSON.stringify({
      card_id:card_id,
    ...
    axios
      ...
      .then((response)=> {
        ...
        navigate('webView',{responseUrl:response.data.data.transaction.url,garage_id:this.props.route.params.garage_id,customer_id:this.props.route.params.customer_id,products:this.props.route.params.products});
        if (response.data.status == 1) {
    ...
    <TouchableOpacity onPress={this.props.navigation.openDrawer}>
      <Image
        ...
        <Image 
          ...
          />
           <Text style={{alignSelf:"center",color:"white",fontSize:25,fontFamily:"NexaBold"}}>Select Cards</Text>
           <View
      ...
      horizontal={true}
        ...
        renderItem={({item}) => 
            ...
            <TouchableOpacity style={{backgroundColor:"#3F51B5",borderRadius:20,margin:10}}  onPress={() => this.selectCard(item.id,item.customer)}>
