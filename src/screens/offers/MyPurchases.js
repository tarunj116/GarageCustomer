...
export default class MyPurchases extends Component {
  ...
  getQuotations(Status) {
    ...
    const url = `${BASE_URL}/customer/get_orders`;
    const params = {
    ...
    axios
      ...
      .then((response) => {
        ...
        if (response.data.success == true) {
          ...
          this.setState({ myPurchaseArr: response.data.data });
          // navigate('home');
        } else {
              ...
              <View>
                <View style={{ flexDirection: 'row', marginTop: 30 }}>
                  <TouchableOpacity
                    ...
                    <Text style={{ color: this.state.purTextColor, fontFamily: "NexaLight", fontSize: 17 }}>HISTORY</Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={[styles.container1, { backgroundColor: this.state.quoteViewColor, marginTop: "2%", left: 2 }]}
                    onPress={() => this.onTabClick("purchased")}>
                    <Text style={{ color: this.state.quoteTextColor, fontFamily: "NexaLight", fontSize: 17 }}>UPCOMING</Text>
                  </TouchableOpacity>
                </View>
                <FlatList
                  data={this.state.myPurchaseArr}
                  ...
                  renderItem={({ item, index }) => {
                    return (
                      <TouchableOpacity onPress={() => {
                        navigate('MyPurchases3',{orderID:item._id});
                      }}  style={{padding:10,height:80,width:"96%",marginTop:30,backgroundColor:"white",borderTopLeftRadius:20,borderTopRightRadius:20,borderBottomLeftRadius:20,borderBottomRightRadius:20,alignSelf:"center", shadowColor: '#000',shadowOffset: { width: 1, height: 1 },shadowOpacity:  0.4,shadowRadius: 3,elevation: 5}}>
                           <View style={{flexDirection:'row',justifyContent:"space-between"}}>
                          
                            <Text style={{marginTop:10,color:"#3F51B5",fontSize:14,fontFamily:"NexaLight"}}>#{item._id}</Text>
                            <Text style={{marginTop:10,color:"#3F51B5",fontSize:14,fontFamily:"NexaLight"}}>AED {item.orderTotal}</Text>
                            </View>
                            <Text style={{marginTop:10,color:"#3F51B5",fontSize:14,fontFamily:"NexaLight"}}>Order date:  {moment(item.createdAt).format('DD-MM-YYYY')}</Text>
                          </TouchableOpacity>
                      );
                  }}
                />
