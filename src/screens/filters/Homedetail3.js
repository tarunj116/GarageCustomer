...
export default class Homedetail3 extends Component {
  ...
  onTabClick = (id)=> {
    ...
    if(id === 'indoor_tab'){
      ...
      this.getGarageService('indoor');
    }
    
     }
     getGarageService(type = ""){
      ...
      const params_garage = JSON.stringify({
        ...
        "cat_id":this.props.route.params.cat_id,
        "type": type
      });
      ...
      .then((response)=> {
        console.log(response.data.data);
        this.setState({garage_service_arr : response.data.data});
  ...
  />
  <Text style={{color:"#3F51B5",fontSize:14,fontFamily:"NexaLight",marginTop:10,marginBottom:10}}>Garage services:</Text>
  <View style={{flexDirection: 'row'}}>
  <TouchableOpacity
  ...
  />
  <View>
  <Text style={{color:"#3F51B5",fontSize:14,fontFamily:"NexaLight",marginTop:10,marginBottom:10,margin:10}}>CAR SERVICES:</Text>
  <FlatList 
         contentContainerStyle={styles.listContainer}
         
         data={this.state.garage_service_arr}
         horizontal={false}
         numColumns={2}
         keyExtractor= {(item) => {
           return item.id;
         ...
         renderItem={(post) => {
           ...
           return (
             <TouchableOpacity style={styles.card} onPress={() => { navigate('Homedetail4',{garage_id : this.props.route.params.garage_id,service_id:item._id })}}> 
               ...
               <View style={styles.cardFooter}>
                 <View style={styles.socialBarContainer}>
                       ...
                       <Text style={{color:"#3F51B5",fontSize:11,fontFamily:"NexaBold",padding:1}}>{item.name}</Text>
                       <Text style={{color:"#3F51B5",fontSize:11,fontFamily:"NexaLight",padding:1}}>AED {item.price}</Text>
